...
// Logs and terminates the app on error
...
// Only logs a warning but does not throw
#define VK_CHECK_WARN(call)                                             \
    ...
    {                                                                   \
        ...
        if (_vk_result != VK_SUCCESS)                                   \
        {                                                               \
            std::ostringstream _vk_warn;                                \
            _vk_warn << "Vulkan warning: " << #call                     \
                     << " returned " << VkResultToString(_vk_result)    \
                     << " (" << _vk_result << ")";                      \
            Core::Logger::log(Core::LogLevel::WARNING, _vk_warn.str()); \
        }                                                               \
    } while (0)